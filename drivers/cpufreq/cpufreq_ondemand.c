...
static void dbs_init_freq_map_table(struct cpufreq_policy *policy)
{
	...
	top1 = (policy->max + policy->min) / 2;
	top2 = (policy->max + top1) / 2;
	...
	for (j = 0; j < cnt; j++) {
		...
		if (tbl[j].frequency < top2) {
			...
			tblmap[3][j] += MAX((policy->max - tbl[j].frequency)/3, min_diff);
		}
		
		tblmap[4][j] += MAX((policy->max - tbl[j].frequency)/2, min_diff);
	}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (max_load_freq > up_threshold_level[1] * policy->cur) {
		...
		if (FREQ_NEED_BUSRT(policy->cur) && cur_load > up_threshold_level[0]) {
