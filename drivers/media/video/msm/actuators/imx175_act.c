...
#else
#define	IMX175_TOTAL_STEPS_NEAR_TO_FAR			58
#endif
...
static int32_t imx175_poweron_af(void)
{
	...
	mdelay(1);
	rc = gpio_request(imx175_msm_actuator_info->vcm_pwd, "imx105");
	if (!rc)
		...
		struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->step_position_table != NULL) {
		uint16_t i = 0;
		uint16_t imx175_nl_region_boundary1 = 3;
		uint16_t imx175_nl_region_code_per_step1 = 48;
		uint16_t imx175_nl_region_boundary2 = 5;
		uint16_t imx175_nl_region_code_per_step2 = 12;
		...
		for (i = 1; i <= a_ctrl->set_info.total_steps; i++){
...
#endif
			{
				...
				} else if (i <= imx175_nl_region_boundary2) {
					a_ctrl->step_position_table[i] =
						a_ctrl->step_position_table[i-1]
						+ imx175_nl_region_code_per_step2;
				} else {
					a_ctrl->step_position_table[i] =
						a_ctrl->step_position_table[i-1]
...
static int32_t imx175_wrapper_i2c_write(struct msm_actuator_ctrl_t *a_ctrl, int16_t next_lens_position, void *params)
{
	int32_t rc = 0;
...
static const struct i2c_device_id imx175_act_i2c_id[] = {
	{"imx105_act", (kernel_ulong_t)&imx175_act_t},
	{ }
};
...
static int __init imx175_i2c_add_driver(void)
{
	pr_info("%s called\n", __func__);
